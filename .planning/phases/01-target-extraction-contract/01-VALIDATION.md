...
| **Quick run command** | `.venv/bin/python tests/test_targets.py` (39 checks), `.venv/bin/python tests/test_nsf_fmrg_data.py` (13 checks), and `.venv/bin/python tests/test_run_target_extraction.py` (10 checks) — 62 total, all green as of 2026-07-23 (re-run live during this audit; corrects `01-VERIFICATION.md`'s stated 38/61 to the actual 39/62 — plan 01-16 added 6 tests and removed 1 superseded one, net +5 over plan 01-15's 34) |
...
| 01-16-D1 | 16 | 15 | TARGET-01 | G-01-6 (Mechanism A) | `halfmax_edges` clip-excludes boundary-touching RAW runs before `merge_adjacent_runs`, so a merged candidate can never itself touch a y-strip boundary | unit | `.venv/bin/python tests/test_targets.py` (leading/trailing-edge swallow regressions) | ✅ | ✅ green |
| 01-16-D2 | 16 | 15 | TARGET-01 | G-01-6 (Mechanism B) | history-based joint far-AND-small plausibility gate (reuses `MIN_TRACKED_LENGTH_RATIO` + runtime `previous_length_mm`, zero new constants) rejects/accepts lone candidates correctly across far+small/close+small/far+large cases and a full-pipeline single-candidate column | unit | `.venv/bin/python tests/test_targets.py` (4 new regressions) | ✅ | ✅ green |
| 01-16-D3 | 16 | 15 | TARGET-02 | — | all 4 tracks regenerated under Amendment A8; coverage up on every track, fragmentation improved on 3/4, crop-edge symptoms resolved, 10-vs-14 FLAG persistence honestly reported (widened, not chased) | integration + manual_procedural | `scripts/check_targets.py --project_dir .` + `01-16-ORDERING-OUTCOME.md` | ✅ | see Manual-Only |
| 01-17-T1 | 17 | 16 | TARGET-02 | — | `01-SIGNOFF-REQUEST.md` "Current state" re-derived live from a fresh `check_targets.py` run + `manifest.json` read against the Amendment A8 generation (run_id/sha256 hex tokens, per-track valid-bin counts/fractions, 10-vs-14 gap) — no transcription from plan text | other | checker-stdout `grep -qxF` verbatim-line loop + 32-hex/64-hex token match against `manifest.json` | ✅ | ✅ green |
| 01-17-T2 | 17 | 16 | TARGET-02 | — | Contract-in-effect table extended to 9 rows (Amendments A3-A8); figure-inspection guidance and decision set rebuilt against `01-16-ORDERING-OUTCOME.md`; existing width-ordering override preserved verbatim, one new unticked reaffirmation item added | other | grep counts (`[ ]`=5, `[x]`=1, 12 `.png` refs, 9 contract rows) | ✅ | ✅ green |
| 01-18-D1 | 18 | 17 | TARGET-02 | — | reasoned no-external-API `COVERAGE.md` declaration unblocks the api-coverage seal gate | other | `node ~/.claude/gsd-core/bin/gsd-tools.cjs check api-coverage-verify-pre 01` → `block: false` | ✅ | ✅ green |
| 01-18-D2 | 18 | 17 | TARGET-02 | — | `01-UAT.md` G-01-6 annotated with Amendment A8's outcome (`post_fix_note`); `status: failed` and 6/5/1 gap-ledger shape provably unchanged | other | awk positional status check + gap-ledger shape counts | ✅ | ✅ green |
| 01-18-D3 | 18 | 17 | TARGET-01 | WR-01 | `scripts/diagnose_track10_coverage.py` carries an in-file WR-01 historical-baseline disclaimer above `classify_column`; diff is comment-only | other | `py_compile` + `git diff` comment/blank-line filter + `check_targets.py` re-run | ✅ | ✅ green |
...
`01-UAT.md` has grown from 6 to 8 tests since the 2026-07-21 audit; no new numbered UAT test was added since 2026-07-22, but the artifacts Tests 7/8 point to were regenerated in place. Tests 1-3 originally reported `issue` (gaps G-01-1/G-01-2/G-01-3); each has since had a root-cause fix land in a dedicated plan (residual bow → Amendment A3 quartic detrend, plan 01-04; boundary sawtooth → continuity tracking + degenerate-window smoothing, plan 01-05; track 10 crop-edge V-spike and coverage collapse → bead-mask fix + Amendment A5, plans 01-07/01-11) and been re-verified live in `01-VERIFICATION.md`. Test 4 passed. Tests 5 and 6 were answered (option b, and `issue` respectively) against pre-Amendment-A6/A7 generations that have since been superseded by plans 01-13/01-14; per `01-UAT.md`, their answers do not carry forward automatically and were re-asked as Tests 7 and 8 against the Amendment-A7 generation. Test 7's option-(a) override was recorded 2026-07-22 and is preserved verbatim by plan 01-17's regeneration. Test 8 (visual figure sign-off) remains fully open. Gap-closure plans 01-16 (Amendment A8 fix), 01-17 (live regeneration of `01-SIGNOFF-REQUEST.md`), and 01-18 (seal-gate declaration + G-01-6 annotation + WR-01 disclosure) landed since the 2026-07-22 audit; none of them attempted the human visual round by design. `01-SIGNOFF-REQUEST.md` now also carries one new unticked reaffirmation item (confirm Test 7's acceptance still holds at the widened 0.3182mm gap, up from 0.2404mm). These are exactly the two `human_verification` entries in `01-VERIFICATION.md` (`status: human_needed`):
...
| Decide the 10-vs-14 width-ordering FLAG (Test 7, decided 2026-07-22) — reaffirm at widened magnitude | TARGET-02 | Explicit scientific/product judgment call; two further independent, pre-registered, outcome-independent fix cycles (01-13, then 01-14) ran against this defect class since Test 5, plus a fifth cycle (01-16, Amendment A8) since Test 7 was decided — the phase's HONEST-OUTCOME GUARD declines further automated tuning without a new independently-diagnosed root cause | `01-UAT.md` Test 7 / `01-SIGNOFF-REQUEST.md` — original option (a) acceptance recorded 2026-07-22 and preserved verbatim by plan 01-17; new unticked reaffirmation item asks the reviewer to confirm the acceptance still holds now that the gap widened to 0.3182mm (from 0.2404mm) under Amendment A8 | **pending** — original decision recorded; widened-magnitude reaffirmation open, decide via `/gsd-verify-work 1` |
| Visual sign-off on 12 regenerated QA figures (Amendment A8 generation) | TARGET-02 | Whether the boundary-tracking improvements (coverage up on every track, fragmentation improved on 3/4) and residual Mechanism-C far-edge jitter are within acceptable bounds is a domain judgment, not a mechanical check | `01-UAT.md` Test 8 / `01-SIGNOFF-REQUEST.md` (regenerated live by plan 01-17) — open all 12 `processed_data/targets/qa/*.png` (run_id `b3f79f207cc1431fa238bb153c04419b`) and answer the 4 questions | **pending** — decide via `/gsd-verify-work 1` |
...
**Approval:** validated 2026-07-20, re-confirmed 2026-07-21 across all 12 plans, re-confirmed 2026-07-22 across all 15 plans, re-confirmed 2026-07-23 across all 18 plans — all mechanical requirements (TARGET-01, TARGET-02) have automated coverage that runs green (62/62 tests re-run live this audit, `check_targets.py` and the api-coverage seal gate both green). Remaining sign-off is scientific/visual judgment, tracked separately as live UAT tests in `01-UAT.md` (not a Nyquist gap) — the phase is currently `human_needed` per `01-VERIFICATION.md`, awaiting a `/gsd-verify-work 1` round, not further automated coverage.
...
| Metric | Count |
|--------|-------|
| Gaps found | 1 (documentation drift: Per-Task Map, Test Infrastructure, and Manual-Only had not been updated since 2026-07-22 and were missing all 8 tasks/waves from plans 01-16 through 01-18 — one boundary-tracking fix (Amendment A8, plan 01-16), one sign-off-document regeneration (plan 01-17), and one seal-gate/annotation/disclosure gap-closure plan (plan 01-18) landed in the interim) |
| Resolved | 1 (Per-Task Map extended with 8 new rows sourced from plans 01-16/01-18's `coverage:` frontmatter and plan 01-17's `## Verification` section — 01-17 lacks `coverage:` frontmatter, so its rows were built from its documented grep/hex-token verification commands instead; Manual-Only reconciled to note Tests 7/8's target artifacts were regenerated under Amendment A8 by plan 01-17, and that a new unticked width-ordering reaffirmation item exists in `01-SIGNOFF-REQUEST.md` alongside Test 8's still-fully-open visual round. All three suites re-run live during this audit — corrected the actual counts to `test_targets.py` 39/39 PASS (not the 38 `01-VERIFICATION.md` stated; plan 01-16 added 6 tests and removed 1 superseded one, net +5 over 34), `test_nsf_fmrg_data.py` 13/13 PASS, `test_run_target_extraction.py` 10/10 PASS — true total 62/62, not 61/61. This is a reporting discrepancy in `01-VERIFICATION.md`'s count only; every test still exits 0 and no requirement lacks coverage. `check_targets.py --project_dir .` re-run live: ALL CHECKS PASSED. The phase's only open item is the human visual sign-off round (`status: human_needed`), which is a UAT gate, not a Nyquist gap.) |
| Escalated | 0 |
