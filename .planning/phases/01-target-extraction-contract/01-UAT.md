...
- gap_id: G-01-6
  ...
  post_fix_note: "Plan 01-16 landed Amendment A8 (see 01-16-ORDERING-OUTCOME.md), closing Mechanism A (clip-exclusion reordered before run merging) and Mechanism B (the history-based far-AND-small plausibility gate against previous_length_mm). Per 01-16-ORDERING-OUTCOME.md, valid-bin coverage rose on every track and contiguous-run fragmentation improved on three of four tracks (marginally worse on one), and both crop-edge symptoms this gap's own reason field named are resolved. Mechanism C's residual far-edge instability was deliberately NOT addressed and remains the already-deferred DP/Viterbi joint-tracker gap. Plan 01-17 regenerated 01-SIGNOFF-REQUEST.md against the live Amendment A8 run so a fresh visual round can be held against the current figures. This note is NOT a sign-off: this gap keeps its current unresolved verdict until a human records a /gsd-verify-work 1 round."
