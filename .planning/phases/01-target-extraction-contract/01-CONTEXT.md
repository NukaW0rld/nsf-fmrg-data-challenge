...
### Amendment A8 (gap-closure -- plan 01-16, dated 2026-07-22)

Amendment A8 closes UAT gap G-01-6: boundary overlays remained fragmented (track 10 severely, tracks 14/21 with frequent EKG-like jumps, track 8 with conspicuous excursions near ~24-27, 48-55, 81-86, 97-100mm) and crop-edge behavior did not get full sign-off (track 10's right band had only an isolated valid sample; track 21's right band ended with an implausibly abrupt terminal drop) even after Amendment A7's merge+plausibility-gate fix. `.planning/debug/boundary-fragmentation-crop-edge-post-A7-visual-signoff.md` diagnosed THREE concurrent mechanisms directly on the real 4-track data. This amendment supersedes nothing in A3-A7.

**Mechanism A (merge-before-clip ordering, dominant driver, 65-90% of `no_candidates` invalidations across all four tracks):** `halfmax_edges` ran `merge_adjacent_runs` *before* the D-01/D-03 boundary-clip-exclusion filter. When a raw edge-touching run (correctly excludable on its own) sat within `MAX_RUN_MERGE_GAP_PIXELS` of an otherwise-valid interior run, the merge fused them and the whole fused span was discarded as edge-touching -- silently swallowing a legitimate candidate. Severity correlated with each track's own chronic y-strip-edge threshold-exceedance (track 10's y-index-0 leading edge: 94.8% of columns above threshold; track 21's y-index-479 trailing edge: 91.4%). The fix reorders `halfmax_edges`: the D-01/D-03 clip-exclusion test is now applied to each RAW run (`all_true_runs(above)`'s output) BEFORE `merge_adjacent_runs` is ever called, so merging only ever combines already-non-edge-touching runs and a merged candidate can structurally never itself touch a y-strip boundary. This introduces no new constant, only a reordering of the two existing steps.

**Mechanism B (same-column-relative gate structurally moot in single-candidate columns, measured 14-72% of tracked selections across the four tracks):** Amendment A7's `MIN_TRACKED_LENGTH_RATIO` same-column gate provides no protection when a column has exactly one (small/spurious) candidate to compare against itself -- that candidate trivially satisfies its own ratio. Directly traced at Track 8's x=24.1/24.5/24.7/25.5mm episodes. The fix adds a history-based joint far-AND-small gate: when tracking history exists, a lone same-column-plausible candidate is rejected only if it is BOTH farther from `previous_center` than the recently-tracked run's own physical width (`previous_length_mm`) AND smaller than `MIN_TRACKED_LENGTH_RATIO` of that same width; otherwise selection proceeds as before. This reuses the already-locked `MIN_TRACKED_LENGTH_RATIO` for the "small" test and derives the "far" distance scale from the just-computed `previous_length_mm` itself -- a runtime-derived quantity, not a new named module constant. `extract_targets_from_arrays` now threads a second piece of sequential state, `previous_length_mm`, alongside `previous_center`: both are set together after a successful column and reset together to `None` after `MAX_TRACKING_GAP_COLUMNS` consecutive invalid columns, exactly as `previous_center` alone did before this amendment. A candidate close to `previous_center` is never rejected on size alone (plausible narrowing of the tracked bead), and a candidate as large as recent tracked history is never rejected on distance alone (plausible drift/relocation) -- both directly regression-tested.

**Mechanism C (greedy nearest-to-`previous_center` selection among multiple simultaneously-plausible candidates, e.g. track 21's tail 1.52mm hop):** re-examined by the diagnosis and, per its own HONEST-OUTCOME GUARD finding, again found not tractable within this phase's remaining scope -- it is the DP/Viterbi joint-tracker gap 01-14-PLAN.md already named and deferred. This amendment does NOT implement a fix for Mechanism C; its residual effect is expected to persist and is reported honestly in `01-16-ORDERING-OUTCOME.md`, not chased.

Both mechanisms apply identically to all four tracks with no per-track constant or conditional -- `test_single_parameterization_has_no_track_conditionals` remains green. Because neither mechanism introduces a new named constant, `extraction_params()`'s existing 19-key set and its provenance digest tests (`test_extraction_params_provenance`, `test_provenance_digest_is_change_sensitive`) are correctly left unchanged -- a deliberate design choice, not an oversight. The no-outcome-driven-tuning prohibition (reaffirmed by Amendments A3-A7) applies identically here: neither `MIN_TRACKED_LENGTH_RATIO`, `MAX_RUN_MERGE_GAP_PIXELS`, nor any other locked constant may be adjusted in response to the resulting fragmentation count, jump statistics, coverage, crop-edge behavior, or the `8 > 10 > 14 > 21` width ordering, and the already-accepted 10-vs-14 width-ordering FLAG (UAT Test 7) is not reopened or re-litigated by this amendment.
